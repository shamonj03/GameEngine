...
public abstract class Renderable {
	/*
	 * I really like cherno's idea of a screen class.
	 * I have heavily taken over this as you can tell.
	 * 
	...
	/**
	 * Clears pixels on renderable.
	 * Sets all current pixels to
	 * black.
	 */
	public void clear() {
	...
	/**
	 * Draws a pixel on the renderable.
	 * Cannot exceed boundary's.
	 * 
	 * @param RGB
	...
	/**
	 * Sets RGB of all other pixels.
	 * Ignores the transparency pixels.
	 * 
	 * @param RGB
	...
	public void offsetRenderableRGB(int redOff, int greenOff, int blueOff) {
		for (int i = 0; i < pixels.length; i++) {
			...
			setPixel(i, RGB16BitInteger.offsetRGB(rgb, redOff, greenOff,
					blueOff));
		}
	}
	...
	public void offsetRenderableHSL(int hueOff, float satOff, float lightOff) {
		for (int i = 0; i < pixels.length; i++) {
			...
			setPixel(i, HSL.offsetHSL(rgb, hueOff, satOff,
					lightOff));
		}
	}
	
	public void offsetHSL(int x, int y, int rgb, int hueOff, float satOff, float lightOff) {
		drawPixel(HSL.offsetHSL(rgb, hueOff, satOff,
				lightOff), x, y);
	}
	

	public void offsetHSV(int x, int y, int rgb, float hueOff, float satOff, float value) {
		drawPixel(HSV.offsetHSV(rgb, hueOff, satOff,
				value), x, y);
	}
	

	public void offsetRGB(int x, int y, int RGB, byte redOffset, byte greenOffset, byte blueOffset) {
		drawPixel(RGB16BitInteger.offsetRGB(RGB, redOffset, greenOffset, blueOffset), x, y);
	}
